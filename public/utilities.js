...
// Draw triangle
const drawPath = (ctx, points, closePath, color='#32EEDB') => {
...
// Draw axis
export const calculateFaceAngle = (mesh) => {
    if (!mesh) return {};
...
// Draw the points
export const drawMesh = (prediction, angle, height, width, ctx, state) => {
    ...
    // Draw head pose
    if (state.headPoseEstimation) {
        let pitch, yaw, roll, xmin, xmax, ymin, ymax, size, x1, y1, x2, y2, x3, y3, tdx, tdy;
        
        pitch = angle['pitch'];
        yaw = angle['yaw'];
        roll = angle['roll'];

        xmin = keypoints[234][0];
        xmax = keypoints[454][0];
        ymin = keypoints[10][1];
        ymax = keypoints[152][1];

        xmin = Math.max(0, xmin - Math.abs(xmax - xmin) / 5);
        xmax = Math.min(width, xmax + Math.abs(xmax - xmin) / 5);
        ymin = Math.max(0, ymin - Math.abs(ymax - ymin) / 10);
        ymax = Math.min(height, ymax + Math.abs(ymax - ymin) / 10);

        // console.log(xmin, xmax, ymin, ymax);
        size = Math.abs(xmax - xmin) / 2;
        tdx = (xmin + xmax) / 2;
        tdy = (ymin + ymax) / 2;
        ...
        // Y-Axis pointing above, drawn in green
        x2 = size * (-Math.cos(yaw) * Math.sin(roll)) + tdx;
        y2 = size * (Math.cos(pitch) * Math.cos(roll) - Math.sin(pitch) * Math.sin(yaw) * Math.sin(roll)) + tdy;

        // Z-Axis pointing out of screen, drawn in blue
        x3 = size * (Math.sin(yaw)) + tdx;
        y3 = size * (-Math.cos(yaw) * Math.sin(pitch)) + tdy;

        ctx.lineWidth = 3;
        drawPath(ctx, [[tdx, tdy], [x1, y1]], false, '#FF0000');
        drawPath(ctx, [[tdx, tdy], [x2, y2]], false, '#00FF00');
        drawPath(ctx, [[tdx, tdy], [x3, y3]], false, '#0000FF');
    }
};